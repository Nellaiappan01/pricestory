// src/components/landing/QuickTrack.tsx
"use client";
...
export default function QuickTrack() {
  // ---- 1) call useSession first (always) ----
  const { data: session, status } = useSession();

  // ---- 2) declare all other hooks immediately so hook order is stable ----
  ...
  // ---- 3) early return while session is loading or if not authenticated ----
  // This is safe because we already called useSession above (hook order preserved).
